...
/// Implements getter (+ mut) for the child element assuming the child is of type `T`.
#[deprecated]
#[proc_macro_derive(SingleWidgetOld)]
pub fn derive_single_widget_old(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    ...
    TokenStream::from(quote!(
    impl #impl_generics const SingleWidgetOld<T> for #name #ty_generics #where_clause {
        type Borrowed<'a>
            = &'a T
        where
            T: 'a,
...
#[deprecated]
#[proc_macro_derive(TwoWidgetOld)]
pub fn derive_two_widget_old(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    ...
    TokenStream::from(quote!(
    impl #impl_generics const TwoWidgetOld<S, T> for #name #ty_generics #where_clause {
