...
#[deprecated]
pub struct CrtWidgetOld<T: DynamicWidget> {
    child: T,
    fill_color: TerminalColor,
    bend_strength: f32,
}

impl<T: DynamicWidget> CrtWidgetOld<T> {
    impl_new!(pub const CrtWidgetOld<T>, child: T, fill_color: TerminalColor, bend_strength: f32);

    impl_getters!(pub const child: T, pub const bend_strength: f32);
}

impl<T: DynamicWidget> DynamicWidget for CrtWidgetOld<T> {
...
impl<T: DynamicWidget> const Deref for CrtWidgetOld<T> {
    type Target = T;
...
impl<T: DynamicWidget> const DerefMut for CrtWidgetOld<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.child
    }
}

pub struct CrtWidget<T> {
    child: T,
    fill_color: TerminalColor,
    bend_strength: f32,
}
...
impl<T: DynamicCharacterWidth> DynamicCharacterWidth for CrtWidget<T> {
    fn width_characters(&self) -> usize {
        self.child.width_characters()
    }
}

impl<T: DynamicCharacterHeight> DynamicCharacterHeight for CrtWidget<T> {
    fn height_characters(&self) -> usize {
        self.child.height_characters()
    }
}

impl<T: ToStringData> ToStringData for CrtWidget<T> {
    fn string_data(&self) -> StringData {
        let data = self.child.string_data().data;
...
impl<T> const Deref for CrtWidget<T> {
    type Target = T;
...
impl<T> const DerefMut for CrtWidget<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.child
    }
}
