...
use crate::{
    ...
    widget::{
        DataCell, DynamicWidget, StringData
    },
};
...
impl<T: DynamicWidget> DynamicWidget for CrtWidget<T> {
    ...
    fn string_data(&self) -> StringData {
        ...
        let mut new_data = vec![
            vec![
                DataCell { character: ' ', foreground: TerminalColor::Default, background: self.fill_color }; 
                data[0].len()
            ];
            data.len()
        ];
        ...
        for (y, line) in data.into_iter().enumerate() {
            ...
            for (x, cell) in line.into_iter().enumerate() {
                let mut uv_x: f32 = x as f32 / width as f32 * 2. - 1.;
                let mut uv_y = y as f32 / height as f32 * 2. - 1.;
                uv_x *= 1. - self.bend_strength * uv_y * uv_y;
                uv_y *= 1. - self.bend_strength * uv_x * uv_x;
                let new_x = ((uv_x + 1.) / 2. * width as f32).round() as usize;
                let new_y = ((uv_y + 1.) / 2. * height as f32).round() as usize;
...
pub struct ScanLineWidget<T: DynamicWidget> {
    child: T,
    width: u32,
    speed: u32,
    interlace: bool,
...
impl<T: DynamicWidget> ScanLineWidget<T> {
    impl_new!(pub const ScanLineWidget<T>, child: T, width: u32, speed: u32, interlace: bool, dimming: f32);

    impl_getters!(pub const child: T);
}

impl<T: DynamicWidget> DynamicWidget for ScanLineWidget<T> {
    fn width_characters(&self) -> usize {
        self.child.width_characters()
    }
...
impl<T: DynamicWidget> const Deref for ScanLineWidget<T> {
    type Target = T;
...
impl<T: DynamicWidget> const DerefMut for ScanLineWidget<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.child
    }
}
