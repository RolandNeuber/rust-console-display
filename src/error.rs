...
pub(crate) const FOUND_CONTROL_CHAR: &str = konst::string::str_concat!(&[
    INVARIANT_VIOLATED,
    " Found control character."
]);
pub(crate) const DATA_DOES_NOT_MATCH_DIMENSIONS: &str =
    konst::string::str_concat!(&[
        INVARIANT_VIOLATED,
        " Data does not match specified dimensions."
    ]);
pub(crate) const PIXEL_INDEX_OUT_OF_RANGE: &str =
    konst::string::str_concat!(&[
        INVARIANT_VIOLATED,
        " Pixel index out of range."
    ]);

pub(crate) const OFFSET_SHOULD_BE_0_OR_1: &str =
    "Offset should be 0 or 1.";
pub(crate) const COULD_NOT_CAST_X_COORD: &str =
    "Could not cast x coordinate.";
pub(crate) const COULD_NOT_CAST_Y_COORD: &str =
    "Could not cast y coordinate.";
